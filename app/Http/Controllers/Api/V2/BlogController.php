...
public function blog_list(Request $request)
{
    ...
    // Comment Count যোগ করা হলো
    $blogs->withCount('comments');
    ...
    $blogs = $blogs->withCount('comments')
               ->where('status', 1)
               ->orderBy('created_at', 'desc')
               ->paginate(12);

    // recent blogs এও চাইলে comment count দিতে পারো
    $recent_blogs = Blog::withCount('comments')
                        ->where('status', 1)
                        ->orderBy('created_at', 'desc')
                        ->limit(9)
                        ->get();
