...
class AuthController extends Controller
{
    ...
    public function login(Request $request)
    {
        //  Log::info('request',[ $request->all()]);
        $messages = array(
            'email.required' => $request->email_or_phone == 'email' ? translate('Email is required') : translate('Phone is required'),
        ...
        //         ->where(function ($query) use ($req_email,$req_phone) {
        //             $query->where('email', $req_email)
        //                 ->orWhere('phone', $req_phone);
        //         })
        //         ->first();
    ...
    public function getUserInfoByAccessToken(Request $request)
{
    ...
    return $this->loginSuccess($user, $request->bearerToken());
}
